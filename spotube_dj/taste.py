...
def record_like(track: dict) -> None:
    ...
    _remember(state, "liked", {
        ...
        # the video id travels with the judgement, because a loved row is a thing a
        # person wants to *see* (its cover) and replay, and neither is possible from
        # a title: `library_view` can look up art by id and `play_row` can play it
