...
mix-blend-mode:screen;opacity:.62;will-change:transform,filter;
animation:tintdrift 8s ease-in-out infinite alternate}
/* the playing colour breathes: the cover-derived tint drifts across the screen and
   its hue wanders a touch, so the wash is alive rather than a fixed gradient.
   8s a side - clearly moving, but still ambient rather than the page sliding under
   the song. The wash is coloured even when idle (see backdrop), so the glow is
   visible the whole time and the animation is never waiting on a cover. */
@keyframes tintdrift{
