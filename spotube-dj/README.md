...
* **one panel cannot take the page down.** Every region in `draw(s)` is called
  ...
  loudest.
* **artwork is warmed on its own lane.** A state read never waits for a
  download: rows that have a file on disk get an `/art/...` href, rows that do not
  draw a palette tile, and the background fetcher fills the rest (64 px for a row,
  220 px for the tile - measured live: 1.9 KB at 1.2 ms per row from disk versus
  21 KB at 30 ms for the `hqdefault.jpg` the Tk skin used to be asked to shrink).
...
before the page opens. Measured on this machine with a live queue of 24: the page is
66 KB and answers in **1.7 ms**, `/api/state` is 8.3 KB in **2.31 ms** (median of 20),
and `tests/test_web.py` is 146 tests (`web-preview.html` in the repo root is the same
page with a live queue inlined, art included).
...
| `tests/test_dj_core.py` | 167 tests: queue, taste, search shape, YTM parsing, cache-first start, auto-mix, station seeds, repeat and shuffle, the self-filling queue, the end-of-song rules, the clear verb, the cache race |
...
| `tests/test_webapp_page.py` | 40 tests: the tick's per-panel guard, the page's shape, id and verb contracts, the `redraw`/`setText` guards and `region()`/`fail()` run under `node`, `node --check`, and that no Tk module comes back |
| `tests/test_brain_config.py` | 57 tests: LLM errors, timeouts, saved config, query hygiene |
| `tests/test_covers.py` | 25 tests: album lookup, rate limiting, the art cache, per-slot artwork sizes, no-ffmpeg behaviour |
| `tests/test_desktop.py` | 15 tests: the .desktop file and the icon, in a temp $XDG_DATA_HOME |
| `tests/test_web.py` | 152 tests: the clear-queue verb and its job lane, the published snapshot, every routed action, the transport verbs, library rows, artwork lanes, path traversal, the Host guard, the settings route, one real socket over the routes |

```bash
python3 -m unittest discover -s tests -t .     # from the repo root: 542 tests
```
