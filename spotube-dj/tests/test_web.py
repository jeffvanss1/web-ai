...
class ArtStampingTests(unittest.TestCase):
    ...
    def test_only_already_cached_art_is_stamped(self):
        self.ctx._hrefs = {"vid0": "/art/a.png", "vid1": "/art/b.jpg"}
    ...
    def test_now_playing_prefers_the_bigger_file(self):
        # rows are 64px and the tile is 220px; one URL for both is what made the
        # first paint slow - shrinking has to happen at the source, not in the UI
        self.ctx._hrefs = {"vid0": "/art/row.png", "vid1": "/art/next.png"}
        self.ctx._bigs = {"vid0": "/art/big.png"}
        s = web.with_art(web.build_state(self.ctx), self.ctx)
        self.assertEqual(s["now"]["art"], "/art/big.png")
        self.assertEqual(s["up_next"][0]["art"], "/art/next.png",
                         "a row must not pull the 220px file")

    def test_big_tile_falls_back_to_the_row_href(self):
        self.ctx._hrefs = {"vid0": "/art/row.png"}
        s = web.with_art(web.build_state(self.ctx), self.ctx)
        self.assertEqual(s["now"]["art"], "/art/row.png")
...
class CoverLandingTests(unittest.TestCase):
    ...
    def test_the_notifier_stores_hrefs_for_both_sizes(self):
        self.ctx._cover_ready("vid1", "row", "/tmp/vid1-caa-64.jpg")
        self.assertEqual(self.ctx.art_href("vid1"), "/art/vid1-caa-64.jpg")
        self.ctx._cover_ready("vid1", "big", "/tmp/vid1-caa-220.jpg")
        self.assertEqual(self.ctx.art_href("vid1", big=True), "/art/vid1-caa-220.jpg")
    ...
    def test_a_cover_landing_later_replaces_the_video_frame(self):
        self.ctx._store_href("vid2", "/tmp/vid2-yt-64.jpg")
        first = self.ctx.art_href("vid2")
        self.ctx._cover_ready("vid2", "row", "/tmp/vid2-caa-64.jpg")
        self.assertEqual(self.ctx.art_href("vid2"), "/art/vid2-caa-64.jpg")
